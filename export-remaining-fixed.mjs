  ...
  try {
    ...
    for (const table of remainingTables) {
      try {
        ...
        // Use literal SQL without interpolation
        let data;
        switch (table) {
          case 'ai_drafts':
            data = await sql`SELECT * FROM ai_drafts ORDER BY id`;
            break;
          case 'ai_jobs':
            data = await sql`SELECT * FROM ai_jobs ORDER BY id`;
            break;
          case 'audit_log':
            data = await sql`SELECT * FROM audit_log ORDER BY id`;
            break;
          case 'media':
            data = await sql`SELECT * FROM media ORDER BY id`;
            break;
          case 'password_reset_tokens':
            data = await sql`SELECT * FROM password_reset_tokens ORDER BY id`;
            break;
          case 'user_cruises':
            data = await sql`SELECT * FROM user_cruises`;
            break;
          default:
            data = [];
        }
