...
interface EnhancedTripInfoSectionsTableProps {
  data: any[];
  columns: TableColumn[];
  actions?: TableAction[];
  keyField?: string;
...
export function EnhancedTripInfoSectionsTable({
  data,
  columns,
  actions = [],
  keyField = 'id',
...
}: EnhancedTripInfoSectionsTableProps) {
  ...
  const { columnWidths, sortConfig, updateColumnWidth, handleSort, sortData } = useTableState(
    'trip_info_sections_table',
    defaultColumnWidths,
    'title' // Default sort by title column (first column after image)
  );
