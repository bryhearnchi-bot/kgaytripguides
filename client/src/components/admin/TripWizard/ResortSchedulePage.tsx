...
export function ResortSchedulePage() {
  const { state, setScheduleEntries, updateScheduleEntry, addScheduleEntry } = useTripWizard();
  ...
  const handleImageUpload = (index: number, url: string) => {
    updateScheduleEntry(index, { imageUrl: url });
  };

  const handleDescriptionChange = (index: number, description: string) => {
    updateScheduleEntry(index, { description });
  };
  ...
  const handleAddDay = () => {
    ...
    const dateExists = state.scheduleEntries.some(entry => entry.date === selectedDate);
    if (dateExists) {
  ...
  const sortedEntries = [...state.scheduleEntries].sort(
    ...
    <div className="space-y-2.5 max-w-3xl mx-auto" ref={entriesContainerRef}>
          ...
          >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
              ...
              <div className="space-y-1">
                <Label className="text-xs font-semibold text-white/90">Day Image (Optional)</Label>
                <ImageUploadField
                  label=""
      ...
      <div className="mt-2.5 p-3 rounded-lg bg-cyan-400/5 border border-cyan-400/20">
        <p className="text-[11px] text-white/70 leading-relaxed">
          <span className="font-semibold text-cyan-400">AI Tip:</span> Add images and descriptions
          for each day of the resort stay. These will appear in the schedule tab on the trip guide
          page.
        </p>
      </div>
            ...
            <div className="space-y-1">
              ...
              <DatePicker
                ...
                disabledDates={date => {
                  ...
                  const day = String(date.getDate()).padStart(2, '0');
                  const dateString = `${year}-${month}-${day}`;
                  return state.scheduleEntries.some(entry => entry.date === dateString);
                }}
                {...(addDayType === 'before'
