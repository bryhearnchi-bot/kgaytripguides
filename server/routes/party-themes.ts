...
export function registerPartyThemeRoutes(app: Express) {
  ...
  app.get(
    ...
    asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
      ...
      if (search) {
        ...
        query = query.or(
          `name.ilike.%${sanitized}%,short_description.ilike.%${sanitized}%,long_description.ilike.%${sanitized}%`
        );
  ...
  app.post(
    '/api/party-themes',
    requireContentEditor,
    validateBody(createPartyThemeSchema),
    asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
      const supabaseAdmin = getSupabaseAdmin();
      ...
      const { data: theme, error } = await supabaseAdmin
        ...
        .insert({
          ...
          amazon_shopping_list_url:
            req.body.amazonShoppingListUrl === '' ? null : req.body.amazonShoppingListUrl,
        })
        .select()
        .single();
  ...
  app.put(
    ...
    requireContentEditor,
    validateParams(idParamSchema as any),
    validateBody(updatePartyThemeSchema),
    asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
      const id = parseInt(req.params.id ?? '0');
      const supabaseAdmin = getSupabaseAdmin();
      ...
      if (req.body.longDescription !== undefined)
        updateData.long_description = req.body.longDescription;
      if (req.body.shortDescription !== undefined)
        updateData.short_description = req.body.shortDescription;
      if (req.body.costumeIdeas !== undefined) updateData.costume_ideas = req.body.costumeIdeas;
      if (req.body.imageUrl !== undefined)
        updateData.image_url = req.body.imageUrl === '' ? null : req.body.imageUrl;
      if (req.body.amazonShoppingListUrl !== undefined)
        updateData.amazon_shopping_list_url =
          req.body.amazonShoppingListUrl === '' ? null : req.body.amazonShoppingListUrl;
    ...
    asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
      ...
      if (usedEvents && usedEvents.length > 0) {
        throw ApiError.conflict(
          'Cannot delete party theme as it is being used by one or more events'
        );
      }
