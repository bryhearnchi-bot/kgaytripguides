...
export function registerAdminLookupTablesRoutes(app: Express) {
  ...
  app.get("/api/admin/lookup-tables/:table", requireAuth, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (!supabaseAdmin) {
      throw new ApiError(503, 'Database service is not configured. Please set up Supabase credentials.');
    }
  ...
  app.post("/api/admin/lookup-tables/:table", requireSuperAdmin, auditLogger('admin.lookup_tables.create'), asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const tableKey = req.params.table as keyof typeof TABLES;
    const tableConfig = TABLES[tableKey];
    ...
    if (existingItem) {
      throw ApiError.conflict(`${tableConfig.displayName.slice(0, -1)} with this name already exists`);
    }
    ...
    if (createError) {
      ...
      throw ApiError.internal(`Failed to create ${tableConfig.displayName.slice(0, -1).toLowerCase()}`);
    }
  ...
  app.put("/api/admin/lookup-tables/:table/:id", requireSuperAdmin, auditLogger('admin.lookup_tables.update'), asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const tableKey = req.params.table as keyof typeof TABLES;
    ...
    if (conflictingItem) {
      throw ApiError.conflict(`Another ${tableConfig.displayName.slice(0, -1).toLowerCase()} with this name already exists`);
    }
    ...
    if (updateError) {
      ...
      throw ApiError.internal(`Failed to update ${tableConfig.displayName.slice(0, -1).toLowerCase()}`);
    }
  ...
  app.get("/api/admin/lookup-tables/counts", requireAuth, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    // Check if user has viewer role or above
    const userRole = req.user?.role;
    const allowedRoles = ['viewer', 'content_manager', 'admin', 'super_admin'];
    if (!userRole || !allowedRoles.includes(userRole)) {
