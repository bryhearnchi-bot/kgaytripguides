...
export function registerAdminUsersRoutes(app: Express) {
  ...
  app.get("/api/admin/users", requireTripAdmin, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    logger.info('=== USERS API CALLED ===');
    logger.info('Query params:', req.query);
    const query = querySchema.parse(req.query);

    if (!supabaseAdmin) {
      throw ApiError.serviceUnavailable('Database service is not configured. Please set up Supabase credentials.');
    }
    ...
    if (error) {
      ...
      throw ApiError.internal('Failed to fetch users', error instanceof Error ? error : undefined);
    }
  ...
  app.post("/api/admin/users", requireTripAdmin, auditLogger('admin.user.create'), asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (!supabaseAdmin) {
      throw ApiError.serviceUnavailable('User management service is not configured. Please set up Supabase credentials.');
    }
    ...
    if (fetchError || !newUser) {
      ...
      if (createError) {
        ...
        await supabaseAdmin.auth.admin.deleteUser(authUser.user.id).catch((err: any) =>
    ...
    if (updateError) {
      logger.error('[User Creation] Failed to update profile with additional fields:', updateError, {
        method: req.method,
        path: req.path
      });
  ...
  app.put("/api/admin/users/:id", requireTripAdmin, auditLogger('admin.user.update'), asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  app.patch("/api/admin/users/:id/status", requireTripAdmin, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const userId = req.params.id;
  ...
  app.delete("/api/admin/users/:id", requireSuperAdmin, auditLogger('admin.user.delete'), asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const userId = req.params.id;
  ...
  app.put("/api/admin/profile", requireAuth, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (error) {
      ...
      throw ApiError.internal('Failed to update profile', error instanceof Error ? error : undefined);
    }
