...
import { validateBody, bulkTalentAssignSchema } from '../middleware/validation';
...
export function registerMediaRoutes(app: Express) {
    ...
    asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
      ...
      if (categoryStats) {
        for (const stat of categoryStats) {
          const categoryData = stat as { talent_categories?: { category?: string } };
          const categoryName = categoryData.talent_categories?.category;
  ...
  app.get(
    ...
    asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
      ...
      const supabaseAdmin = getSupabaseAdmin();
      let query = supabaseAdmin.from('talent').select(`
        id,
        name,
        talent_category_id,
        bio,
      ...
      const transformedResults = (results || []).map(talent => {
        const talentData = talent as {
          id: number;
          name: string;
          ...
          created_at: string;
          updated_at: string;
          talent_categories?: { category?: string };
        };
        return {
          id: talentData.id,
          name: talentData.name,
          talentCategoryId: talentData.talent_category_id,
          bio: talentData.bio,
          knownFor: talentData.known_for,
          profileImageUrl: talentData.profile_image_url,
          socialLinks: talentData.social_links,
          website: talentData.website,
          createdAt: talentData.created_at,
          updatedAt: talentData.updated_at,
          category: talentData.talent_categories?.category,
        };
      });
  ...
  app.get(
    ...
    asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
      ...
      const talentData = talentItem as {
        id: number;
        name: string;
        talent_category_id: number;
        bio: string | null;
  ...
  app.get(
    ...
    asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
      ...
      // Transform results to match expected format
      const transformedResults = (talentList || []).map(talent => {
        const talentData = talent as {
          id: number;
          name: string;
          talent_category_id: number;
        ...
        return {
          ...
          updatedAt: talentData.updated_at,
          category: talentData.talent_categories?.category,
          role: talentData.trip_talent?.role,
        };
      });
  ...
  app.post(
    ...
    asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
      ...
      logger.info(
        `Initial talentCategoryId: ${talentCategoryId}, Type: ${typeof talentCategoryId}`
      );
      ...
      if (
        !talentCategoryId ||
        talentCategoryId === 0 ||
        isNaN(Number(talentCategoryId)) ||
        Number(talentCategoryId) <= 0
      ) {
  ...
  app.put(
    ...
    asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
      ...
      if (req.body.talentCategoryId !== undefined)
        updateData.talent_category_id = req.body.talentCategoryId;
      ...
      if (req.body.profileImageUrl !== undefined)
        updateData.profile_image_url =
          req.body.profileImageUrl === '' ? null : req.body.profileImageUrl;
      if (req.body.socialLinks !== undefined) updateData.social_links = req.body.socialLinks;
      if (req.body.website !== undefined)
        updateData.website = req.body.website === '' ? null : req.body.website;
  ...
  app.post(
    ...
    asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
      ...
      const { data: resetData, error: resetError } = await supabaseAdmin.rpc('sql', {
  ...
  app.post(
    '/api/trips/:tripId/talent/:talentId',
    requireContentEditor,
    asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
      const tripId = parseInt(req.params.tripId || '0');
      ...
      const supabaseAdmin = getSupabaseAdmin();
      const { error } = await supabaseAdmin.from('trip_talent').upsert({
        trip_id: tripId,
        talent_id: talentId,
        role: req.body.role,
      });
