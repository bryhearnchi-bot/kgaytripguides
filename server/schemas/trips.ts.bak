...
import { z } from 'zod';
import { idParamSchema, slugParamSchema, paginationSchema, searchSchema, sortingSchema, dateRangeSchema, timeSchema, urlSchema, priceSchema } from './common';
...
export const tripFilterSchema = paginationSchema
  ...
  .extend({
    ...
    isFeature: z.string().transform(val => val === 'true').optional(),
 ...
 * Event type enum
 */
export const eventTypeEnum = z.enum(['party', 'show', 'dining', 'lounge', 'fun', 'club', 'after', 'workshop', 'meetup']);
...
export const createEventSchema = z.object({
  ...
  // Talent
  talentIds: z.array(z.number().int().positive())
  ...
  data => !data.endTime || data.time < data.endTime,
...
export const eventFilterSchema = paginationSchema
  ...
  .extend({
    ...
    requiresReservation: z.string().transform(val => val === 'true').optional(),
    hasAvailability: z.string().transform(val => val === 'true').optional(),
...
export const exportTripSchema = z.object({
  ...
  includeData: z.array(
    z.enum(['basic', 'itinerary', 'events', 'talent', 'media', 'pricing', 'bookings'])
